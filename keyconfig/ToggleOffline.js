...
window.forcedCache = window.forcedCache || {
	f: {
		...
		"script.google.com/": [/\w{32}\.cache\.js/, /\w{32}\/3\.cache\.js/, /\w{32}\/6\.cache\.js/, /\d+-CallbackCss_ltr\.css/, /\d+-maestro_ide_shell_bin_i18n_maestro_ide_shell\.js/, /\d+-code_mirror_bin_code_mirror.js/, /\d+-MaestroIdeShellCss_ltr.css/],
...
if(!window.forcedCache.r) {
	...
	cache.asyncVisitStorage({
		...
		onCacheEntryVisitCompleted: function() {
			...
			if(diff > 0)
				alert('Warning: ' + diff + ' Fiverr cache entries missing');
		}
	}, true);
}
